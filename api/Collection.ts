...
export class Collection<Request, Response> {
	...
	constructor(private readonly url: string, key: string, private signer?: string, private algorithm?: authly.Algorithm) {
		this.headers = {
	...
	private async getError(response: FetchResponse): Promise<gracely.Error> {
		return gracely.server.backendFailure({ status: response.status, body: response.headers.get("Content-Type")?.includes("json") ? await response.json() : await response.text() })
	}
}
